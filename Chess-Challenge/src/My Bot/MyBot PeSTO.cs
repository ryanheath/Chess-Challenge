...
// class EvilBot : IChessBot
{
    ...
    int Evaluate()
    {
        ...
        {
            if (pieceList.TypeOfPieceInList == PieceType.None) continue;
            var colorIndex = pieceList.IsWhitePieceList ? 0 : 1;
            foreach (var piece in pieceList)
            {
                var sq = piece.Square.Index;
                sq = piece.IsWhite ? sq ^ 0b111 : sq;
                var pieceTypeIndex = (int)piece.PieceType - 1;
                mg[colorIndex] += mg_table[pieceTypeIndex][sq] + mg_value[pieceTypeIndex];
                eg[colorIndex] += eg_table[pieceTypeIndex][sq] + eg_value[pieceTypeIndex];
                gamePhase += gamephaseInc[pieceTypeIndex];
            } 
        }

        /* tapered eval */
        var mgScore = mg[0] - mg[1];
        var egScore = eg[0] - eg[1];
        var mgPhase = gamePhase;
        if (mgPhase > 24) mgPhase = 24; /* in case of early promotion */
        var egPhase = 24 - mgPhase;
        return (board.IsWhiteToMove ? 1 : -1) * (mgScore * mgPhase + egScore * egPhase) / 24;
    }
    ...
    static readonly int[] gamephaseInc = { 0, 1, 1, 2, 4, 0 };
    static readonly int[] mg_value = { 82, 337, 365, 477, 1025, 0 };
    static readonly int[] eg_value = { 94, 281, 297, 512,  936, 0 };
    static readonly int[][] mg_table =
    {
        new [] // pawn
        {
            0,   0,   0,   0,   0,   0,  0,   0,
